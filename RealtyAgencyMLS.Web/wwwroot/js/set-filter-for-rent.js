$(document).ready(function () {
    ...
    $("#mob_daysonrealty").on("change", function (e) {
...
function saleRentClick(paramName, paramValue, viewType) {
    setGetParameter(paramName, paramValue);
    if (paramValue === "For Rent")
        window.location.href = `${baseUri}/ForRent/${viewType}/?sale_rent=` + paramValue;
    else
        window.location.href = `${baseUri}/ForSell/${viewType}/?sale_rent=` + paramValue;
}
...
function GetPropertyDetails(propertyID, propUrl, viewType) {
    $('#staticBackdrop').modal('hide');
    $('#propDetails').html('');
    if (!window.location.href.includes('Details')) {
        localStorage.setItem("propertyListInfoLocal", window.location.href);
    }
    $.ajax({
        url: `${baseUri}/ForRent/Details/${propertyID}/${propUrl}/${viewType}/?isModal=true`,
        type: "get",
        success: function (response) {
            $('#propDetails').html(response);
            $('#staticBackdrop').modal('show');
            window.history.pushState('1', "Property Details", `${baseUri}/ForRent/Details/${propertyID}/${propUrl}/${viewType}/`);
        },
        error: function () {
...
function goBack() {
    window.history.pushState('1', "Property Details", localStorage.getItem("propertyListInfoLocal"));
    $('#staticBackdrop').modal('hide');
